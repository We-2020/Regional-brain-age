...
from mydata1F import dataGenerator,get_Dataframe_Data
...
# 配置日志输出格式和级别  
logging.basicConfig(filename='SQET.log', level=logging.INFO,   
...
#     scheduler =  CosineAnnealingLR(optimizer, T_max=15, eta_min=1e-5)
    ...
    for epoch in range(epochs):
        ...
        for imgs,age in tqdm(train_loader,ncols=50):
            imgs = imgs.float()
            imgs, age = imgs.to(device), age.float().to(device)
            optimizer.zero_grad()
            pred = model(imgs.float())['final'].float().squeeze()
    ...
    #             torch.save(model.state_dict(),'net_params.pth.')
                torch.save(model.module.state_dict(), f"SQET{k}.pth")
        scheduler.step(test_loss)
    print('best,best_M',bMAE,bMSE)
    Du.append([k,bMAE,bMSE,bR,bR2])
print(Du)
with open('SQET.txt','w',encoding='utf8') as f:
