...
from mydata2 import dataGenerator,get_Dataframe_Data
...
# 配置日志输出格式和级别  
logging.basicConfig(filename='SFCN.log', level=logging.INFO,   
...
for k in range(1,6):
    ...
    #             torch.save(model.state_dict(),'net_params.pth.')
                torch.save(model.module.state_dict(), f"SFCN{k}.pth")
        scheduler.step()
    print('best,best_M',bMSE,bMAE)
    Du.append([k,bMAE,bMSE,bR,bR2])
print(Du)
with open('SFCN.txt','w',encoding='utf8') as f:
