...
# 配置日志输出格式和级别  
logging.basicConfig(filename='attention_unet4.log', level=logging.INFO,   
...
for k in range(1,6):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.7, patience=2, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=1e-7, eps=1e-08)
#     scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.7, patience3, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=1e-6, eps=1e-08) 最优不加torch.manual_seed(3407)
#     scheduler =  CosineAnnealingLR(optimizer, T_max=16, eta_min=1e-6)
    ...
    #             torch.save(model.state_dict(),'net_params.pth.')
                torch.save(model.module.state_dict(), f"attention_unet{k}.pth")
        scheduler.step(test_loss)
    print('best,best_M',bMAE,bMSE)
    Du.append([k,bMAE,bMSE,bR,bR2])
print(Du)
with open('attention_unet4.txt','w',encoding='utf8') as f:
