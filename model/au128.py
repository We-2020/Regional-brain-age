...
class UNetWithTransformerEncoder(nn.Module):
    def __init__(self, d_model, nhead, num_layers, dim_feedforward, dropout):
        ...
        self.pool = nn.AdaptiveMaxPool3d((8,8,8))  # 全局平均池化
        self.regressor = nn.Linear(8*8*8*8*5, 1)
        self.liner = nn.Linear(d_model,8)
        self.grelu = QuickGELU()
        # Transformer编码器层的设置
    ...
    def forward(self, x):
        ...
        d2 = self.adjust_d2(d2)
        d3 = self.adjust_d3(d3)
        d4 = self.adjust_d4(d4)
        d5 = self.adjust_d5(d5)
        e5 = self.adjust_e5(e5)
        ...
        # print(e5.shape,d5.shape,d4.shape,d3.shape,d2.shape)
...
#         e5 = self.adjust_e5(F.interpolate(e5, size=d4.shape[2:], mode='trilinear', align_corners=True))
        ...
        # 合并特征
        combined_features = torch.cat([d2, d3, d4, d5, e5], dim=1)
        # Transformer融合
